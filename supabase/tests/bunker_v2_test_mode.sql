...
select ok(
  pg_get_functiondef('public.owner_bunker_v2_test_simulate_current(uuid)'::regprocedure)
    ~ '_bunker_v2_apply_m05_outcome\([^;]+,false\)',
  'M05 rehearsal simulation is a deliberate decision, not a timeout fallback'
);
