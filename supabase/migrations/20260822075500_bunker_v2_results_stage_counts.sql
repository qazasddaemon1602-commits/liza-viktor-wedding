...
as $$
  ...
  from required
  left join instance_counts using(mission_code);
$$;
revoke all on function public._bunker_v2_mission_stage_counts(uuid,uuid)
...
begin
  ...
  select
    count(*) filter(where profile.character_status='active')::integer,
  ...
  where archive.event_id=v_event_id
    and archive.run_nonce=v_state.run_nonce
    and archive.decryption_status='decoded';

  select
    coalesce(sum(lot.quantity) filter(where lot.status='available'),0)::integer,
  ...
  where transfer.event_id=v_event_id
    and transfer.run_nonce=v_state.run_nonce
    and transfer.status='accepted';

  if v_final.id is not null then
    select count(*)::integer into v_wrong
    from public.bunker_mission_decisions decision
    where decision.instance_id=v_final.id
      and decision.decision_key like 'final_access_%'
      and coalesce((decision.outcome->>'success')::boolean,false)=false;
  end if;

  select count(*)::integer into v_skills
  from public.bunker_ability_uses ability
  where ability.event_id=v_event_id
    and ability.run_nonce=v_state.run_nonce
    and ability.status='committed';
  ...
  select exists(
    ...
    where instance.event_id=v_event_id
      ...
      and instance.mission_code='MISSION_06'
      and instance.outcome->>'status'='success'
  ) into v_m06_success;

  v_coordination:=greatest(0,least(100,
    case
      when v_missions_total>0
        then round(40.0*v_missions_completed/v_missions_total)::integer
      else 0
    end
  ...
  return jsonb_build_object(
    ...
    'missionsTotal',coalesce(v_missions_total,6),
    'coordinationScore',v_coordination
  );
end;
$$;

revoke all on function public.get_bunker_v2_results(text)
  from public,anon,authenticated;
grant execute on function public.get_bunker_v2_results(text)
