...
begin
  ...
  select r, t.event_id, t.state into v_a, v_event_id, v_state
  from public.mk_registrations r
  ...
  if v_a.id is null or v_b.id is null or v_a.tournament_id <> v_b.tournament_id then
  ...
  update public.mk_registrations set seed=null, updated_at=now() where id in (v_a.id, v_b.id);
...
begin
  ...
  into v_registration, v_event_id, v_state
  from public.mk_registrations r
  join public.mk_tournaments t on t.id=r.tournament_id
