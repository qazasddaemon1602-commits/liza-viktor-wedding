-- Durable guest-facing Bunker V2 dashboard projection.
-- Mission projections stay transient; this RPC reads only authoritative long-lived ledgers.
...
begin
  ...
  from (
    select
      ...
      coalesce(sum(lot.quantity) filter (where lot.status = 'transferred'), 0)::integer as transferred,
  ...
  from (
    select distinct on (entry.id)
      ...
      entry.acquired_at,
      case
        when entitlement.owner_scope_kind = 'global' then 'global'
        else 'wagon'
      end as scope,
      case
        when entry.decryption_status = 'locked' then '{}'::jsonb
        else entry.content
      end as content
    ...
    order by entry.id,
      case when entitlement.owner_scope_kind = 'global' then 0 else 1 end
  ) archive;
