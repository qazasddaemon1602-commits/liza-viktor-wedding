create or replace function public.get_registration_carriage_map(p_event_slug text)
returns jsonb
language plpgsql
stable
security definer
...
begin
  ...
  select coalesce(
    pg_catalog.jsonb_agg(
      ...
      order by ordered_carriage.sort_order, ordered_carriage.number, ordered_carriage.id
    ),
    '[]'::jsonb
  )
  into v_carriages
      ...
      coalesce(
        (
          select pg_catalog.jsonb_agg(
            pg_catalog.jsonb_build_object(
              'id', ordered_guest.id,
              'fullName', ordered_guest.full_name,
          ...
          from (
            select
              ...
              coalesce(
                nullif(
                  pg_catalog.concat_ws(
                    ' ',
                    nullif(pg_catalog.btrim(guest.first_name), ''),
                    nullif(pg_catalog.btrim(guest.last_name), '')
                  ),
                  ''
                ),
                'Гость'
              ...
              coalesce(
                nullif(
                  pg_catalog.left(
                    pg_catalog.upper(
                      pg_catalog.concat(
                        pg_catalog.left(pg_catalog.regexp_replace(guest.first_name,'[^[:alpha:]]','','g'),1),
                        pg_catalog.left(pg_catalog.regexp_replace(guest.last_name,'[^[:alpha:]]','','g'),1)
                      )
                    ),
                    2
                  ),
              ...
              pg_catalog.row_number() over (
                order by guest.registered_at, guest.ticket_sequence, guest.id
              ) as seat_index
            from public.guests guest
            where guest.event_id = v_event_id
  ...
  return pg_catalog.jsonb_build_object(
    'status', case
      when v_map_expected_guest_count > 0
        and v_registered_guest_count >= least(v_map_expected_guest_count, 40)
        then 'complete'
      else 'registration'
    end,
...
revoke all on function public.get_registration_carriage_map(text) from public;
grant execute on function public.get_registration_carriage_map(text) to anon, authenticated;
