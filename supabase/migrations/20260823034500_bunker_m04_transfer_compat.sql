...
begin
  ...
  if v_transfer_lot_text is null and v_transfer_item_key is null then
  ...
  if v_transfer_lot_text is not null
    and v_transfer_lot_text !~* '^[0-9a-f]{8}-[0-9a-f]{4}-[1-8][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$' then
    raise exception 'invalid Mission 04 inventory item' using errcode = '22023';
  end if;
  if v_transfer_item_key is not null
    and v_transfer_item_key !~ '^[a-z][a-z0-9_]+$' then
    raise exception 'invalid Mission 04 inventory item' using errcode = '22023';
  end if;
  if v_transfer_to_text is null
  ...
  if v_transfer_lot_text is not null then
  ...
  if v_transfer_lot_id is not null then
    ...
    if v_transfer_item_key is not null
      and v_transfer_item_key <> v_source.item_key then
      raise exception 'invalid Mission 04 transfer item mismatch' using errcode = '22023';
    end if;
  else
    select item.* into v_source
    from public.bunker_inventory_lots item
