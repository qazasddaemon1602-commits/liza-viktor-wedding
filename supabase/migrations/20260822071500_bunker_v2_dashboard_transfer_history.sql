-- Correct persistent dashboard transfer history for partial M04 transfers.
-- Accepted transfer rows are the authoritative quantity ledger for outgoing items.
...
begin
  ...
  with item_keys as (
    ...
    where lot.event_id = v_event_id
      and lot.run_nonce = v_state.run_nonce
    ...
    where transfer.event_id = v_event_id
      ...
      and transfer.from_carriage_id = v_guest.carriage_id
  ...
  select coalesce(
    jsonb_agg(
      ...
      ) order by archive.acquired_at, archive.artifact_key
    ),
    '[]'::jsonb
  ) into v_archive
  from (
    select distinct on (entry.id)
      ...
      entry.artifact_key,
      entry.content_type,
      entry.decryption_status,
      entry.acquired_at,
      case when entitlement.owner_scope_kind = 'global' then 'global' else 'wagon' end as scope,
      case when entry.decryption_status = 'locked' then '{}'::jsonb else entry.content end as content
    ...
    where entitlement.event_id = v_event_id
      ...
      and (
        ...
        or (entitlement.owner_scope_kind = 'wagon' and entitlement.carriage_id = v_guest.carriage_id)
      )
...
revoke all on function public.get_guest_bunker_v2_dashboard(text,text)
  from public, anon, authenticated;
grant execute on function public.get_guest_bunker_v2_dashboard(text,text)
