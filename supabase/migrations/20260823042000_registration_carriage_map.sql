...
begin
  ...
  where guest.event_id = v_event_id
    and guest.affiliation_detail is distinct from '__BUNKER_TEST__';
  ...
  return pg_catalog.jsonb_build_object(
    'status', case
      when v_expected_guest_count > 0
        and v_registered_guest_count >= least(
          v_expected_guest_count,
          40
        )
        then 'complete'
      else 'registration'
    end,
    'expectedGuestCount', v_expected_guest_count,
