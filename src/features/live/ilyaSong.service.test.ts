...
describe('Ilya song service', () => {
  ...
  it('sends only the requested owner action to the song control RPC', async () => {
    ...
    await expect(controlIlyaSong(client, 'liza-viktor', 'play')).resolves.toEqual({
      status: 'ok', eventId: 'song-1', action: 'play',
    });
    expect(calls).toEqual([{
      name: 'owner_control_ilya_song',
      args: { p_event_slug: 'liza-viktor', p_action: 'play' },
    }]);
  ...
  it('delivers valid realtime song events and unsubscribes cleanly', () => {
    ...
    receive?.({
      new: {
        ...
        kind: 'ilya_song',
        created_at: '2026-08-26T18:00:00Z',
        payload: { action: 'play', title: 'Песня про Илью', artist: 'Посажёный отец', durationMs: 233080 },
      },
    });
