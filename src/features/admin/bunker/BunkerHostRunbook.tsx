...
const INTERMISSIONS: Record<IntermissionKey, RunbookContent> = {
  PROLOGUE: {
    ...
    host: {
      brief: 'Объясните правила один раз: гости играют командами вагонов, один участник отправляет общее решение с телефона, реальные люди никогда не выбывают.',
      say: [
        'Добро пожаловать на поезд «Последний вагон». Сегодня каждый стол становится отдельной командой.',
      ...
      doNotRevealUntil: [
        'Не называйте BK-17, Сектор 04, Бункер и архивную последовательность.',
  ...
  BREAK: {
    ...
    eyebrow: 'ПЕРЕРЫВ · ПЕРВАЯ МЕТКА BK-17',
    host: {
      brief: 'Дайте гостям выдохнуть и раскройте только первую зацепку: исключённые записи связаны с объектом BK-17.',
      say: [
        'Пассажирский протокол принят. В удалённых записях повторяется одна служебная метка: BK-17.',
        'Мы пока не знаем, что это — станция, состав или закрытый объект.',
      ...
      doNotRevealUntil: [
        ...
        'Не подтверждайте ни одну догадку гостей о назначении объекта.',
      ],
