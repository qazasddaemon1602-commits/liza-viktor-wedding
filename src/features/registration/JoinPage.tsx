    ...
    return (
      <main className="registration-shell registration-ticket-surface">
        <section className="registration-card registration-join-error" role="alert">
          ...
          <button className="registration-submit" type="button" onClick={() => void restore()}>
            ПОПРОБОВАТЬ СНОВА
          </button>
        </section>
      </main>
  ...
  if (guest) {
    return (
      <GuestHub
        ...
        bunkerDashboard={bunker.dashboard}
        bunkerDashboardError={bunker.dashboardError}
        bunkerMissionOne={bunker.missionOne}
        bunkerMissionTwo={bunker.missionTwo}
        bunkerMissionThree={bunker.missionThree}
        bunkerMissionFour={bunker.missionFour}
        bunkerMissionFive={bunker.missionFive}
        bunkerMissionSix={bunker.missionSix}
        bunkerUnknownPassenger={bunker.unknownPassenger}
        bunkerFinal={bunker.final}
        ...
        onConfirmBunkerMissionOne={(passengerIds) => bunker.confirmMissionOne(passengerIds)}
        onSubmitBunkerMissionTwo={(answers) => bunker.submitMissionTwo(answers)}
        onUseBunkerMissionTwoAbility={(answer) => bunker.useMissionTwoAbility(answer)}
        onConfirmBunkerMissionThree={(problems) => bunker.confirmMissionThree(problems)}
        onUseBunkerMissionThreeAbility={(problem) => bunker.useMissionThreeAbility(problem)}
        onSendBunkerMissionFourMessage={(message) => bunker.sendMissionFourMessage(message)}
        onProposeBunkerMissionFourTrade={(input) => bunker.proposeMissionFourTrade(input)}
        onRespondBunkerMissionFourTrade={(tradeId, response) => bunker.respondMissionFourTrade(tradeId, response)}
        onSubmitBunkerMissionFourAnswer={(answer) => bunker.submitMissionFourAnswer(answer)}
        onCastBunkerMissionFiveVote={(vote) => bunker.castMissionFiveVote(vote)}
        onUseBunkerMissionFiveAbility={() => bunker.useMissionFiveAbility()}
        onRevealBunkerMissionSixFragment={() => bunker.revealMissionSixFragment()}
        onCastBunkerMissionSixVote={(vote) => bunker.castMissionSixVote(vote)}
        onUseBunkerMissionSixAbility={() => bunker.useMissionSixAbility()}
        onRequestBunkerFinalAccess={(values) => bunker.requestFinalAccess(values)}
