...
const ITEM_GUIDES: Record<string, ItemGuide> = {
  ...
  water: { label: 'Запас воды', purpose: 'Поддерживает людей и помогает при перегреве.', asset: '/images/bunker/items/water.webp' },
  ...
  return inventory
    .filter((row) => (
      typeof row.available === 'number'
        ? row.available > 0
        : row.status === 'available' && Number(row.quantity ?? 0) > 0
    ))
    .map(itemKey)
    .filter(Boolean);
}
...
export function BunkerInventoryCards({ inventory, missionContent }: InventoryProps) {
  ...
  return (
    <div className="bunker-player-list bunker-inventory-cards" aria-label="Инвентарь вагона">
      {inventory.map((row, index) => {
        const key = itemKey(row);
        const guide = itemGuide(row, missionContent);
        const isAggregate = typeof row.available === 'number';
        const isAvailable = isAggregate
        ...
        return (
          <article key={typeof row.id === 'string' ? row.id : `${key || 'item'}-${index}`}>
            ...
            <div>
              <h3>{guide.label}</h3>
              {isAggregate ? (
                <div className="bunker-inventory-card__counts">
                  <span>Доступно: {Number(row.available ?? 0)}</span>
                  <span>Использовано: {Number(row.used ?? 0)}</span>
                  <span>Передано: {Number(row.transferred ?? 0)}</span>
                  <span>Потеряно: {Number(row.lost ?? 0)}</span>
                </div>
              ) : (
