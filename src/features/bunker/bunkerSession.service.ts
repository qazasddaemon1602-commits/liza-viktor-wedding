...
import { parseBunkerV2State, type BunkerV2State } from "./v2/contracts";
...
export type BunkerContractState =
  { contractVersion: 1; state: BunkerGlobalGameState } | BunkerV2State;
const UUID =
...
function validWagonCount(v: unknown): v is number {
  return nonNegativeInteger(v) && v >= 2 && v <= 5;
}
function gameState(v: unknown): BunkerGlobalGameState {
  if (typeof v !== "string" || !GAME_STATES.has(v as BunkerGlobalGameState))
    throw new Error("Unexpected Bunker game state");
  return v as BunkerGlobalGameState;
}
export function parseBunkerContractState(v: unknown): BunkerContractState {
  if (!record(v)) throw new Error("Unexpected Bunker contract version");
  if (v.contractVersion === 2) return parseBunkerV2State(v);
  if (v.contractVersion !== 1)
    throw new Error("Unexpected Bunker contract version");
  return { contractVersion: 1, state: gameState(v.state) };
}
function missionPlan(v: unknown): BunkerMissionPlan {
  if (v === null || v === undefined) return null;
  if (typeof v !== "object") throw new Error("Unexpected Bunker mission plan");
...
export function parseBunkerCurrentMission(
  ...
  if (v === null || v === undefined) return null;
  if (!record(v) || typeof v.id !== "string" || !v.id.trim())
    throw new Error("Unexpected Bunker current mission");
  const state = gameState(v.state);
  if (state !== authoritativeState)
    throw new Error("Unexpected Bunker mission state transition");
  return { id: v.id, state, plan: missionPlan(v.plan) };
}
function throwRpc(e: Exclude<BunkerRpcError, null>): never {
  if (e instanceof Error) throw e;
  const next = new Error(e.message || "Bunker session request failed");
  if (e.code) Object.assign(next, { code: e.code });
  throw next;
}
async function rpc(
...
function parseAdvanced(data: unknown): AdvancedBunkerGameState {
  ...
  const previousState = gameState(data.previousState),
    globalGameState = gameState(data.globalGameState);
  return {
    status: "transitioned",
...
export async function prepareBunkerGame(
  client: BunkerRpcClient,
  eventId: string,
  gameMode: BunkerGameMode = "production",
) {
  return parsePrepared(
    await rpc(client, "owner_prepare_bunker_game", {
      p_event_id: eventId,
...
export async function distributeBunkerCharacters(
  ...
  return parseDistributed(
    await rpc(client, "owner_distribute_bunker_characters", {
      p_event_id: eventId,
    }),
  );
}
export async function advanceBunkerGameState(
  ...
  return parseAdvanced(
    await rpc(client, "owner_advance_bunker_game_state", {
      p_event_id: eventId,
