...
export function BunkerPlayerDashboard({runtime,connectionError='',missionOne,missionTwo,missionThree,missionFour,missionFive,missionSix,unknownPassenger,onConfirmMissionOne,onSubmitMissionTwo,onUseMissionTwoAbility,onConfirmMissionThree,onUseMissionThreeAbility,onSendMissionFourMessage,onProposeMissionFourTrade,onRespondMissionFourTrade,onSubmitMissionFourAnswer,onCastMissionFiveVote,onUseMissionFiveAbility,onRevealMissionSixFragment,onCastMissionSixVote,onUseMissionSixAbility}:Props){
  ...
  const hasMission=Boolean(missionOne||missionTwo||missionThree||missionFour||missionFive||missionSix||unknownPassenger);
  ...
  return <section className={`bunker-player-dashboard${hasMission?' bunker-player-dashboard--active-mission':''}`} aria-label="Игровой модуль Бункер">
    ...
    {runtime.character.status==='excluded'&&<p className="bunker-player-dashboard__continuity" role="status">Персонаж исключён из истории, но вы продолжаете участвовать.</p>}
    ...
    <div className="bunker-player-dashboard__content">
      ...
      {section==='ПАССАЖИРЫ'&&<div className="bunker-player-list">{passengers.length?passengers.map(passenger=><article key={String(passenger.guestId)}><h3>{String(passenger.realName)}</h3><strong>{String(passenger.profession)}</strong><p>{String(passenger.visibleSkill)}</p></article>):<article><h3>СОСТАВ ВАГОНА</h3><p>Список появится после синхронизации.</p></article>}</div>}
      {section==='ИНВЕНТАРЬ'&&<div className="bunker-player-list">{inventory.length?inventory.map((item,index)=><article key={`${String(item.itemKey)}:${index}`}><h3>{bunkerItemLabel(String(item.itemKey))}</h3>{item.status&&<strong>{bunkerStatusLabel(String(item.status))}</strong>}<p>Количество: {String(item.quantity)}</p></article>):<article><h3>ИНВЕНТАРЬ ПОКА ПУСТ</h3></article>}</div>}
      {section==='АРХИВ'&&<article><h3>АРХИВ ВАГОНА</h3>{archive.length?<div>{archive.map(entry=>{const image=artwork(entry),label=bunkerArchiveLabel(entry.artifactKey);return <article key={`${entry.scope}:${entry.artifactKey}`}>{image&&<BunkerResponsivePicture asset={image} className="bunker-player-archive__artwork" testId="bunker-archive-artwork"/>}<h3>{label.title}</h3><strong>{bunkerContentTypeLabel(entry.contentType)}</strong><p>{label.hint}</p></article>;})}</div>:<p>Найденные материалы появятся здесь автоматически.</p>}</article>}
      {section==='СОСТОЯНИЕ'&&(isV2?<article><h3>СОСТОЯНИЕ ВАГОНА</h3><p>Системы меняются от решений команды и сохраняются на сервере.</p></article>:<article><h3>СОСТОЯНИЕ ВАГОНА</h3><p>ПИТАНИЕ · {bunkerStatusLabel(String(runtime.wagonState.powerStatus))}</p><p>СВЯЗЬ · {bunkerStatusLabel(String(runtime.wagonState.communicationStatus))}</p><p>НАВИГАЦИЯ · {bunkerStatusLabel(String(runtime.wagonState.navigationStatus))}</p></article>)}
