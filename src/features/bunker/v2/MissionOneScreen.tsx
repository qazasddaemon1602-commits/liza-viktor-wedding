...
export type MissionOneScreenWagon = { wagonId: string; label: string; status: 'active' | 'completed' };
export type MissionOneScreenReadModel = { title: string; publicSummary: string; remainingSeconds: number; wagons: readonly MissionOneScreenWagon[] };
    ...
    <section className="bunker-mission-one-screen" aria-label="Задание 1 · общий экран">
      ...
      <main className="bunker-mission-one-screen__main">
        ...
        {model.wagons.length > 0 ? (
          <ul className="bunker-mission-one-screen__wagons" aria-label="Прогресс вагонов" data-count={model.wagons.length}>
            {model.wagons.map((wagon, index) => (
              <li key={wagon.wagonId} className={wagon.status === 'completed' ? 'is-complete' : ''}>
                <span>{String(index + 1).padStart(2, '0')}</span><strong>{wagon.label}</strong>
      ...
      <footer><span>АРХИВ БУНКЕРА</span><span>ПУБЛИЧНЫЙ ПРОГРЕСС · В СЕТИ</span></footer>
    </section>
  );
}
