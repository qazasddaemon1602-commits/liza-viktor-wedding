...
const active = {
  ...
  attemptsRemaining: 2,
  selectedAnswers: ['', '', ''],
  ability: { available: true, key: 'terminal_hack', label: 'Работа со служебным терминалом', hint: 'Можно запросить подсказку.' },
} as const;
...
const screenModel = {
  ...
  wagons: [{ wagonId: 'w1', label: 'ВАГОН №1', status: 'active', attemptCount: 1 }],
} as const;
...
const ownerModel = {
  ...
  deadlineAt: active.deadlineAt,
  title: active.title,
  wagons: [{ wagonId: 'w1', label: 'ВАГОН №1', status: 'active', attemptCount: 1, hintsUsed: 1 }],
} as const;

describe('Mission 02 read model', () => {
  it('parses exactly six public evidence items without leaking the answer key', async () => {
    const model = await getGuestMissionTwoReadModel(
  ...
  it('validates TV and owner projections separately, including owner-only hint counts', async () => {
    await expect(getMissionTwoScreenReadModel(
      { rpc: vi.fn().mockResolvedValue({ data: screenModel, error: null }) },
      'liza-viktor',
    )).resolves.toEqual(screenModel);
    await expect(getOwnerMissionTwoReadModel(
      { rpc: vi.fn().mockResolvedValue({ data: ownerModel, error: null }) },
      'event-1',
    )).resolves.toEqual(ownerModel);
    ...
    await expect(getOwnerMissionTwoReadModel(
      { rpc: vi.fn().mockResolvedValue({ data: { ...ownerModel, wagons: [{ ...ownerModel.wagons[0], hintsUsed: -1 }] }, error: null }) },
      'event-1',
    )).rejects.toThrow(/hints used/i);
  });
});
...
describe('Mission 02 commands', () => {
  it('submits the three answers through the authoritative command transport only', async () => {
    const rpc = vi.fn().mockResolvedValue({ data: {
      ...
      commandId: '52000000-0000-4000-8000-000000000020', commandType: 'submit_answer',
    }, error: null });
