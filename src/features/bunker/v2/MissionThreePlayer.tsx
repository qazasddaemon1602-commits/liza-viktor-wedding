...
export function MissionThreePlayer({ model, onConfirm, onUseAbility }: {
  ...
  const useAbility = () => {
    if (!model.ability?.available || busy) return;
    setBusy(true);
    Promise.resolve(onUseAbility?.(model.ability.problemKey)).finally(() => setBusy(false));
  };
  const confirm = () => {
    if (!onConfirm || selected.length === 0 || busy) return;
    setBusy(true);
    Promise.resolve(onConfirm([...selected])).finally(() => setBusy(false));
  };
          ...
          <section aria-label="Проблемы вагона">
            ...
            <div className="bunker-v2-problems">
              {model.problems.map((problem) => {
                ...
                const disabled = !captain || !available || (!selectedProblem && selected.length >= 3);
                return <label key={problem.key} className={selectedProblem ? 'is-selected' : ''}>
                  ...
                  <span><strong>{problem.title}</strong><small>{problem.risk}</small><em>Подходит: {bunkerItemLabel(problem.itemKey)}</em>{!available && <small>Недоступно: требуется {bunkerItemLabel(problem.itemKey).toLocaleLowerCase('ru-RU')}.</small>}</span>
                </label>;
              })}
            </div>
          </section>
          {captain && <button className="bunker-v2-mission__primary" type="button" disabled={selected.length === 0 || busy || !onConfirm} onClick={confirm}>ПОДТВЕРДИТЬ РАСПРЕДЕЛЕНИЕ</button>}
          {model.ability?.available && <aside className="bunker-v2-mission__ability"><strong>ВАША СПОСОБНОСТЬ МОЖЕТ ПОМОЧЬ</strong><p>{model.ability.label} — для проблемы «{model.problems.find((problem) => problem.key === model.ability?.problemKey)?.title}».</p><button type="button" disabled={busy} onClick={useAbility}>ПРИМЕНИТЬ МОЮ СПОСОБНОСТЬ</button></aside>}
